...
Upstash Redis (`UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN`), sorted set
`medlens:waitlist:entries` — works on Vercel's serverless functions (stateless
REST client, no persistent connection) without needing the full Supabase
schema stood up yet. This reuses the same Upstash Redis instance the rest of
the architecture already plans for caching/idempotency, just a different key.
Migrates to Supabase once the tenant/report schema exists.
