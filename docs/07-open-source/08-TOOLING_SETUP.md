...
- Security updates and version updates both target `main` directly now — no retargeting workflow needed (the old two-branch model required one; see `04-BRANCHING_STRATEGY.md` for why that was dropped).
...
Trunk-based, single branch — see [`04-BRANCHING_STRATEGY.md`](./04-BRANCHING_STRATEGY.md). Every PR (including Dependabot's) targets `main` directly and merges via squash; merge manually after review.

### Manual cleanup after removing Codacy

See [`docs/06-ops/06-SUPPLY_CHAIN_SECURITY.md`](../06-ops/06-SUPPLY_CHAIN_SECURITY.md#codacy-removal-manual-ui) for the full checklist:
...
- **Secret:** `GH_PAT` — see [`docs/06-ops/05-GITHUB_AUTOMATION_PAT.md`](../06-ops/05-GITHUB_AUTOMATION_PAT.md)
...
- **Recommended PR size:** ≤150 changed files for useful AI review; ≤400 lines meaningful diff for human review (see [`10-PR_REVIEW_AND_PROTECTION_POLICY.md`](./10-PR_REVIEW_AND_PROTECTION_POLICY.md))
...
See [`docs/06-ops/04-BRANCH_PROTECTION_SETUP.md`](../06-ops/04-BRANCH_PROTECTION_SETUP.md).
