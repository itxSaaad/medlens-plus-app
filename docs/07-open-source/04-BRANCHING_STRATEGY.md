...
4. `ci.yml` runs quality checks on the resulting push to `main`, then calls `release.yml` (semantic-release: tag + GitHub Release) and `deploy.yml` as `needs:`-gated reusable workflows in the same run. Anything not ready for users yet ships behind a feature flag (see `docs/06-ops/02-CONFIGURATION_AND_FLAGS.md`), not behind a separate branch.

## Why trunk-based

This repo ran a two-branch (`main` + `develop`) GitFlow-lite model for a while. It caused a real, recurring problem: keeping two independently-protected branches in sync — same code _and_ same commit history — turned out to be structurally difficult on GitHub, for reasons worth knowing if this is ever reconsidered:
...
`develop` served as a staging/integration gate before code reached `main`. Trunk-based development replaces that function with feature flags: land code on `main` behind a flag, verify it in whatever environment you need, flip the flag when ready. See `docs/06-ops/02-CONFIGURATION_AND_FLAGS.md`.
...
- One-time setup: [`docs/06-ops/04-BRANCH_PROTECTION_SETUP.md`](../06-ops/04-BRANCH_PROTECTION_SETUP.md)
- Branch, commit, and PR naming: [`03-NAMING_CONVENTIONS.md`](./03-NAMING_CONVENTIONS.md)
- Release process: [`11-RELEASE_PROCESS.md`](./11-RELEASE_PROCESS.md)
