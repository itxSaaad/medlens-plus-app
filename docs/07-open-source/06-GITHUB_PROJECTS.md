...
GitHub does not expose a public API to create views. Follow [`07-PROJECT_VIEWS_SETUP.md`](./07-PROJECT_VIEWS_SETUP.md) (step-by-step GUI guide).

## Maintainer sync (not for contributors)

See `docs/06-ops/07-MAINTAINER_DELIVERY.md` — run `.github/maintainer/sync-delivery.py --all` only when board fields need repair.
...
Full rules: [`03-NAMING_CONVENTIONS.md`](./03-NAMING_CONVENTIONS.md)
...
- **AI review:** manual only — `@coderabbitai review` / assign Copilot when ready (see [`08-TOOLING_SETUP.md`](./08-TOOLING_SETUP.md))
- Target: `main` — trunk-based, single branch (see [`04-BRANCHING_STRATEGY.md`](./04-BRANCHING_STRATEGY.md))
