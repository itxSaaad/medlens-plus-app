...
- Repository secret **`GH_PAT`** for automated workflows — see [`05-GITHUB_AUTOMATION_PAT.md`](./05-GITHUB_AUTOMATION_PAT.md)
- No `gh auth login` required — the setup script uses `curl` + PAT from git remote

## Current policy

- **PR required** on `main` (the only long-lived branch — trunk-based, see [`04-BRANCHING_STRATEGY.md`](../07-open-source/04-BRANCHING_STRATEGY.md))
