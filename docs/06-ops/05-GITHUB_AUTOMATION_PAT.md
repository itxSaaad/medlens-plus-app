...
Trunk-based (single `main` branch) removed the cross-branch sync/drift automation (`sync-develop.yml`, `branch-drift.yml`, `dependabot-retarget.yml`) that previously needed this PAT too — see [`04-BRANCHING_STRATEGY.md`](../07-open-source/04-BRANCHING_STRATEGY.md) for why.
...
See also: [`04-BRANCH_PROTECTION_SETUP.md`](./04-BRANCH_PROTECTION_SETUP.md), [`06-SUPPLY_CHAIN_SECURITY.md`](./06-SUPPLY_CHAIN_SECURITY.md).
