...
### Checkpoint: Free-Tier -> Paid-Tier Upgrade Triggers

Revisit before Phase 3 (Public Beta) at the latest — public traffic is the
first thing likely to hit a free-tier ceiling. See
[../architecture/07-FREE_TIER_LIMITS.md](../02-architecture/07-FREE_TIER_LIMITS.md)
for the concrete per-service limits and what breaks first; treat the
Vercel Hobby ToS flag in that doc as a pre-launch blocker, not a scaling
concern.

### Checkpoint: LangGraph Migration

Currently the workflow is a linear class mirroring future node boundaries
(see `apps/api/src/api/workflows/report_processing.py`). Migrating it to a
real `StateGraph` with conditional edges (low-confidence branch to manual
review) is a Phase 2 hardening item, not required for the Phase 1 core
loop. See
[../ai/04-LANGGRAPH_MIGRATION.md](../03-ai/04-LANGGRAPH_MIGRATION.md) for the target
node/state/edge design.

### Modular Growth: What Extends Without a Rewrite

The architecture is deliberately shaped so the four most likely growth
directions are additive, not rewrites. This is the core "modular growth"
promise of the plan — each of these is a bounded change behind an existing
boundary:

- **New lab panel types (beyond LFT/CBC/KFT).** Adding a panel is **data +
  config, not new code paths**: new entries in the hand-curated synonym→LOINC
  table and one new panel-specific extraction call. The extraction pipeline
  already issues one focused LLM call per panel and canonicalizes against the
  synonym table in a deterministic post-pass, so a new panel reuses the same
  code paths — see
  [../ai/01-EXTRACTION_PIPELINE.md](../03-ai/01-EXTRACTION_PIPELINE.md) ("Split by
  panel") and the synonym-table-as-idempotent-data-migration rule in
  [../architecture/02-STACK_DECISIONS.md](../02-architecture/02-STACK_DECISIONS.md).
  Unrecognized biomarkers are captured with `loinc_code = NULL` rather than
  dropped, so new vendor terminology surfaces for mapping instead of failing.

- **Mobile (Expo) app.** It lands **on top of the shared-package boundary
  established now**, not as a later refactor. `packages/api-client`,
  `packages/types`, `packages/hooks` (business logic + query hooks +
  chart-series shaping), and `packages/ui-tokens` are built for web today with
  React as a peer dependency and no `next/*`/DOM imports, so Metro can bundle
  them unchanged. Mobile supplies only its platform layer: native chart
  renderer, NativeWind token consumption, and a SecureStore auth adapter. See
  [../architecture/05-SHARED_PACKAGES_STRATEGY.md](../02-architecture/05-SHARED_PACKAGES_STRATEGY.md)
  and
  [../mobile/01-CROSS_PLATFORM_STRATEGY.md](../04-mobile/01-CROSS_PLATFORM_STRATEGY.md).

- **Free-tier → paid-tier upgrade.** Every externally-bounded service (DB,
  storage, queue, LLM, worker host) sits behind an adapter/factory selected by
  env var, so scaling past a free-tier ceiling is a **config/env change, not a
  code change**: a plan-tier bump plus a key rotation, with the provider swap
  order already policy-defined. Concrete per-service limits and what breaks
  first are in
  [../architecture/07-FREE_TIER_LIMITS.md](../02-architecture/07-FREE_TIER_LIMITS.md);
  the adapter mechanism is in
  [../architecture/02-STACK_DECISIONS.md](../02-architecture/02-STACK_DECISIONS.md)
  ("Provider Swap Policy").

- **Node 5 (Alert/Care) — flagged extension point, not designed here.** The
  5-node graph reserves Node 5 as the designated modular extension point for
  alerting/care workflows per
  [../architecture/01-SYSTEM_ARCHITECTURE.md](../02-architecture/01-SYSTEM_ARCHITECTURE.md).
  It stays intentionally undesigned in this plan; the guarantee is only that
  the graph has a clean seam for it, so adding it later does not disturb the
  Node 1–4 core loop.
